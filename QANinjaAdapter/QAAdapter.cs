...
namespace QANinjaAdapter
{
    public class QAAdapter : AdapterBase, IAdapter, IDisposable
    {
        ...
        public void Connect(IConnection connection)
        {
            ...
            _syntheticStraddleService = new SyntheticStraddleService(this);
        ...
        private async void Connect()
        {
            if (this._ninjaConnection.Status == ConnectionStatus.Connecting)
            {
                if (Connector.Instance.CheckConnection())
                {
                    ...
                    // Load synthetic straddle configurations from Documents\NinjaTrader 8\QAAdapter directory
                    string documentsPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
                    string configFilePath = System.IO.Path.Combine(documentsPath, "NinjaTrader 8", "QAAdapter", "straddles_config.json");
                    _syntheticStraddleService.LoadStraddleConfigs(configFilePath);
                    Logger.Info("SyntheticStraddleService initialized and configurations loaded.");
                }
                else
        ...
        /// <summary>
        /// Checks if an instrument is a synthetic instrument defined in our straddles configuration
        /// </summary>
        private bool IsSyntheticInstrument(string instrumentName)
        {
            if (_syntheticStraddleService == null)
                return false;
                
            // Check if this instrument is a synthetic straddle symbol
            foreach (var state in _syntheticStraddleService.GetStraddleStates())
            {
                if (state.Definition.SyntheticSymbolNinjaTrader.Equals(instrumentName, StringComparison.OrdinalIgnoreCase))
                    return true;
            }
            ...
            Action<MarketDataType, double, long, DateTime, long> callback)
        {
            try
            {
                ...
                bool isSynthetic = IsSyntheticInstrument(name);
                
                if (isSynthetic)
                {
                    // For synthetic instruments, we don't subscribe to the WebSocket directly
                    // Instead, we rely on the constituent legs to generate synthetic ticks
                    Logger.Info($"Detected synthetic instrument subscription: {name}. No direct WebSocket subscription needed.");
                ...
                // Now handle the websocket subscription for real instruments
                lock (QAAdapter._lockLiveSymbol)
                {
                    ...
                    if (!this._marketLiveDataSymbols.Contains(name))
                    {
                        ...
                        Task.Run(async () => {
                            try
                            {
                                // Create a callback for market data events
                                Action<QANinjaAdapter.Models.MarketData.MarketDataEventArgs> tickCallback = (marketDataEvent) => {
                                    // Process the market data event
                                    // This would be where you handle the tick data
                                    // For now, just log it
                                    if (this._ninjaConnection.Trace.Connect)
                                        NinjaTrader.NinjaScript.NinjaScript.Log($"Received market data event for {marketDataEvent.Symbol}, Type: {marketDataEvent.Type}, Price: {marketDataEvent.Price}", NinjaTrader.Cbi.LogLevel.Information);
                                };

                                // Create the active check function
        ...
        /// This method creates a NinjaTrader-compatible MarketDataEventArgs and passes it to NT's data engine.
        /// </summary>
        public void PublishSyntheticTickData(string syntheticSymbol, double price, DateTime timestamp, long volume, TickType tickType)
        {
            ...
            lock (_marketDataLock)
            {
                ...
                // Look for the instrument in the available instruments
                foreach (var instrument in MasterInstrument.All)
                {
                    if (instrument.Name.Equals(syntheticSymbol, StringComparison.OrdinalIgnoreCase))
                    {
                        // Create an Instrument instance from the MasterInstrument
                        ntInstrument = new Instrument { MasterInstrument = instrument };
                        break;
                    }
                }
                ...
                {
                    Logger.Error($"ERROR: Synthetic instrument '{syntheticSymbol}' not found in NinjaTrader's instrument collection. " +
                                "Ensure it's correctly defined in mapped_instruments.json and loaded by NT.");
                    return;
                }

                // 2. Map TickType to NinjaTrader's MarketDataType
                MarketDataType ntMarketDataType = MarketDataType.Last;
                if (tickType == TickType.Bid) ntMarketDataType = MarketDataType.Bid;
                else if (tickType == TickType.Ask) ntMarketDataType = MarketDataType.Ask;
                // Add more mappings if needed for Quote, Trade, etc.

                // 3. Create callback for each subscription
                foreach (var cb in _l1Subscriptions[syntheticSymbol].L1Callbacks.Values)
                {
                    try
                    {
                        // Ensure the timestamp has the correct Kind property set
                        // Convert to local time first to ensure Kind is set to Local
                        DateTime localTime = DateTime.SpecifyKind(timestamp, DateTimeKind.Local);
                        ...
                        try
                        {
                            ...
                            indianTime = TimeZoneInfo.ConvertTime(localTime, TimeZoneInfo.Local, tz);
                            
                            // Log the time conversion
                            NinjaTrader.NinjaScript.NinjaScript.Log(
                                $"[SYNTHETIC-TIME] Original={timestamp:HH:mm:ss.fff} Kind={timestamp.Kind}, " +
                                $"Local={localTime:HH:mm:ss.fff} Kind={localTime.Kind}, " +
                                $"Indian={indianTime:HH:mm:ss.fff} Kind={indianTime.Kind}",
                                NinjaTrader.Cbi.LogLevel.Information);
                        }
                        catch (Exception ex)
                        {
                            // If timezone conversion fails, use local time
                            Logger.Warn($"Failed to convert time to IST: {ex.Message}. Using local time instead.");
                            indianTime = localTime;
                        }

                        // Log synthetic tick
                        NinjaTrader.NinjaScript.NinjaScript.Log(
                            $"[SYNTHETIC-TICK] Publishing for {syntheticSymbol}: Price={price:F2}, Volume={volume}, Time={indianTime:HH:mm:ss.fff}",
                            NinjaTrader.Cbi.LogLevel.Information);

                        // Call the callback with the synthetic tick data
                        cb(ntMarketDataType, price, volume, indianTime, 0L);
                    }
                    catch (Exception ex)
                    {
                        Logger.Error($"Error publishing synthetic tick for {syntheticSymbol}: {ex.Message}", ex);
                    }
                }
            }
        }
        ...
        public void ProcessParsedTick(string nativeSymbolName, ZerodhaTickData tickData)
        {
            try
            {
                ...
                // Check if this tick is for a straddle leg
                if (_syntheticStraddleService != null && _syntheticStraddleService.IsLegInstrument(nativeSymbolName))
                {
                    // Convert ZerodhaTickData to the internal Tick DTO
                    var tick = new Tick
                    {
                        InstrumentSymbol = nativeSymbolName,
                        Price = tickData.LastTradePrice,
                        Volume = tickData.LastTradeQty,
                        Timestamp = tickData.LastTradeTime,
                        Type = TickType.Last // Default to Last, could be refined based on tickData properties
                    };
                    
                    // Process the tick for synthetic straddle calculation
                    _syntheticStraddleService.ProcessLegTick(tick);
                    
                    NinjaTrader.NinjaScript.NinjaScript.Log(
                        $"[SYNTHETIC-STRADDLE] Processed leg tick for {nativeSymbolName}",
                        NinjaTrader.Cbi.LogLevel.Information);
                }
            ...
            IProgress progress)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(instrumentName) && IsSyntheticInstrument(instrumentName))
                {
                    // For synthetic instruments, immediately return empty bars
                    // This avoids any historical data requests to the exchange
                    Logger.Info($"Detected historical data request for synthetic instrument: {instrumentName}. Returning empty bars.");
                    
                    // Call the callback with empty bars and no error
                    callback(bars, ErrorCode.NoError, string.Empty);
                    return;
                }
                
                // For regular instruments, proceed with normal historical data request
